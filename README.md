...
#### 12/04 Fri
+ TSN이 점수가 안나오고, TSM이 점수가 잘나옴

#### 12/07 Mon
+ TSM만 log가 맞지 않다. log 점수는 높게 나오고 실제 테스트는 낮고.
+ 차트가 이상해.

#### 12/08 tues
+ TSM 프레임 8개로 나눠서 하던걸 16개로 나눠서 해보는중이다. 현재 epoch10 좀더 괜찮아보인다.
false alarm. 별 차이가 없다.

#### 12/09 wed
+ 데이터가 6300개나 들어왔다.
+ 8988 오늘까지온 동영상 갯수
+ 전처리 없이 싹다 돌렷다. 63% epoch 35
+ class 21개라서 epoch가 50이 넘어가도 별 나아 지지 않음.
+  
...
[머신러닝 용어집](https://developers.google.com/machine-learning/glossary?hl=ko#pooling)
